...
qjm.prototype = {
  ...
  add_event_click: function add_event_click() {
    ...
    canvas.addEventListener("click", function (e) {
      ...
      var ex = newxy.x;
      var ey = newxy.y;
      var all_nodes = _this2.allNodePosMap; // 点击内容节点
...
qjm.Mind.prototype = {
  ...
  get_expanded_node_group: function get_expanded_node_group() {
    ...
    for (var i = 0; i < t.nodes.length; i++) {
      ...
      (function _rank(obj, rankid) {
        ...
        groupInfo.expandedRankMaxLeft = obj.direction === -1 && rankid > groupInfo.expandedRankMaxLeft ? rankid : groupInfo.expandedRankMaxLeft; // 有展开的子元素，则给下一级push进去他的所有子元素数组
  ...
  get_group_max_length: function get_group_max_length() {
    ...
    for (var i = 0; i < t.nodes.length; i++) {
      var groupInfo = t.nodes[i].groupInfo; // 对比左右两边的末级节点的高度，将最大值赋值给节点组高度
  ...
  layout: function layout() {
    ...
    for (var i = 0; i < t.nodes.length; i++) {
      ...
      var expandedNodesRight = group.groupInfo.expandedNodesRight; // 比较左右两边的高度，值大则反向布局，值小的正向布局
      // 左边

      var rank_max_nodes_length_l = qjm.util.flatArray(expandedNodesLeft[expandedNodesLeft.length - 1]); // 右边
